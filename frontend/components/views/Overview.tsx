...
export default function Overview({
  ...
  const [simulatedCompletion, setSimulatedCompletion] = useState<number | null>(
    null,
  );
  ...
  useEffect(() => {
    const calculateSimulatedCompletion = () => {
      ...
      if (saved && data?.status_counts) {
        try {
          ...
          const totalDocs = Object.values(data.status_counts).reduce(
            (a: any, b: any) => a + b,
            0,
          ) as number;

          if (totalDocs > 0) {
            const explicitTrue = Object.values(parsed).filter(
              (v) => v === true,
            ).length;
            const explicitFalse = Object.values(parsed).filter(
              (v) => v === false,
            ).length;

            if (Object.keys(parsed).length > 0) {
              const adjustedApproved = Math.max(
                0,
                Math.min(
                  totalDocs,
                  baseApproved + explicitTrue - explicitFalse,
                ),
              );
    ...
    return () =>
      window.removeEventListener("storage", calculateSimulatedCompletion);
  }, [data]);
  ...
  const totalDocs = data?.status_counts
    ? (Object.values(data.status_counts).reduce(
        (a: any, b: any) => a + b,
        0,
      ) as number)
    : 0;
  ...
  const pendingDocs =
    (data?.status_counts?.pendente || 0) +
    (data?.status_counts?.correcao_solicitada || 0);

  const cert = data?.certificate || {};
  const certPortaria =
    certForm.portaria || cert.portaria || "Aguardando portaria";
  ...
  const certRenewalWindow =
    cert.renewal_start && cert.renewal_end
  ...
  useEffect(() => {
    ...
    if (data?.certificate) {
      setCertForm({
        portaria: data.certificate.portaria || "",
        validade: data.certificate.valid_until || "",
      });
    }
  }, [data]);
  ...
  const handleDismissPrazo = (id: string, action: "concluido" | "excluido") => {
    ...
    if (onToast)
      onToast(
        action === "concluido"
          ? "Prazo marcado como concluído!"
          : "Alerta removido do painel.",
      );
  };
  ...
  return (
    <>
      <div className="flex flex-col md:flex-row md:items-start justify-between gap-4 mb-8">
        <div>
          <span className="text-[10px] uppercase tracking-widest text-[#879087] font-bold">
            Visão geral · Ciclo {new Date().getFullYear()}
          </span>
          <h1 className="font-serif text-3xl text-[#34332f] mt-1">
            Boa tarde, {firstName}.
          </h1>
          <p className="text-sm text-[#7d837e] mt-1">
            O seu panorama de conformidade do CEBAS Educação.
          </p>
        </div>
        <div className="text-right">
          <span className="text-[10px] uppercase tracking-widest text-[#879087] font-bold block mb-1">
            Período de aferição
          </span>
          ...
          <span className="inline-flex items-center gap-1.5 rounded-full bg-[#e7f3ee] px-2.5 py-1 text-[10px] font-bold text-[#4b8c78]">
            <span className="h-1.5 w-1.5 rounded-full bg-[#4b8c78] animate-pulse"></span>{" "}
            Monitoramento ativo
          </span>
        </div>
      </div>
          ...
          <div className="space-y-6">
                ...
                <div className="flex flex-col md:flex-row md:items-start gap-8">
                  <div className="relative h-32 w-32 shrink-0">
                    ...
                    <div className="absolute inset-0 flex flex-col items-center justify-center">
                      <span className="font-serif text-4xl text-[#34332f] leading-none">
                        {completion}
                      </span>
                      <span className="text-[10px] text-[#879087] mt-1 font-bold">
                        /100
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="flex-1 w-full">
                    ...
                    <h2 className="font-serif text-xl text-[#34332f] mt-2 mb-1">
                      {completion >= 90
                        ? "Estrutura sólida, com atenção pontual."
                        : "O dossiê exige atenção e ajustes."}
                    </h2>
                    <p className="text-xs text-[#879087] mb-6">
                      O índice consolida requisitos legais, evidências e prazos
                      do ciclo.
                    </p>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-5">
                      <div>
                        <div className="flex justify-between text-[11px] mb-2">
                          <span className="text-[#647078]">
                            Regularidade institucional
                          </span>
                          <span className="font-bold text-[#4b8c78]">
                            Conforme
                          </span>
                        </div>
                        <div className="h-1.5 w-full bg-[#f4f2ea] rounded-full overflow-hidden">
                          <div className="h-full bg-[#4b8c78] w-[100%] rounded-full" />
                        </div>
                      </div>
                      <div>
                        <div className="flex justify-between text-[11px] mb-2">
                          ...
                          <span className="font-bold text-[#c49a3c]">
                            Atenção
                          </span>
                        </div>
                        <div className="h-1.5 w-full bg-[#f4f2ea] rounded-full overflow-hidden">
                          <div className="h-full bg-[#c49a3c] w-[75%] rounded-full" />
                        </div>
                      </div>
                      <div>
                        <div className="flex justify-between text-[11px] mb-2">
                        ...
                        <div className="h-1.5 w-full bg-[#f4f2ea] rounded-full overflow-hidden">
                          <div className="h-full bg-[#c49a3c] w-[60%] rounded-full" />
                        </div>
                      </div>
                      <div>
                        <div className="flex justify-between text-[11px] mb-2">
              ...
              <div className="rounded-xl bg-[#292e43] p-6 md:p-8 text-white relative overflow-hidden flex flex-col justify-between shadow-lg">
                ...
                <div className="relative z-10">
                  ...
                  <div className="flex justify-between items-start">
                    <h3 className="text-xs text-[#98a5b8] font-bold uppercase tracking-widest mb-1">
                      {data.institution?.name || "Instituição Logada"}
                    </h3>
                    {!isEditingCert && (
                      <button
                        onClick={() => setIsEditingCert(true)}
                        className="text-[#98a5b8] hover:text-white transition-colors"
                  ...
                  {isEditingCert ? (
                    <form
                      ...
                      className="mt-2 space-y-3 bg-[#313750] p-4 rounded-lg border border-[#50566b]"
                    >
                      <div>
                        <label className="text-[10px] text-[#98a5b8] uppercase tracking-wider block mb-1">
                          Portaria / Despacho
                        </label>
                        <input
                          required
                          type="text"
                          value={certForm.portaria}
                          onChange={(e) =>
                            setCertForm({
                              ...certForm,
                      ...
                      <div className="flex justify-end gap-2 pt-2">
                        <button
                          ...
                          className="text-xs text-[#98a5b8] hover:text-white px-2"
                        >
                          Cancelar
                        </button>
                        <button
                          type="submit"
                          className="bg-[#c49a3c] text-white text-xs px-3 py-1.5 rounded font-bold hover:bg-[#aa7a1d] transition-colors"
                        >
                          Gravar
                        </button>
                      </div>
                    ...
                    <>
                      ...
                      <div className="mt-8 space-y-4 text-[11px]">
                        <div className="flex justify-between items-center border-b border-[#3b4159] pb-3">
                          <span className="text-[#98a5b8] flex items-center gap-2">
                            <Calendar size={12} /> Válido até
                          </span>
                        ...
                        <div className="flex justify-between items-center border-b border-[#3b4159] pb-3">
                          <span className="text-[#98a5b8] flex items-center gap-2">
                            <Calendar size={12} /> Janela de renovação
                          </span>
                ...
                <div className="mt-8 relative z-10">
                  <strong className="font-serif text-3xl text-[#c49a3c]">
                    {certDays}{" "}
                    <span className="text-sm font-sans text-[#98a5b8] font-normal">
                      dias p/ vencimento
                    </span>
                  </strong>
                  <button
            ...
            <div className="grid md:grid-cols-3 gap-6">
              <Card
                ...
                onClick={() => router.push("/gratuidade")}
              >
                ...
                <h3 className="font-serif text-4xl text-[#34332f]">
                  {gratCurrent.toLocaleString("pt-BR")}%
                </h3>
                <p className="text-xs text-[#879087] mt-2">
                  Equivalente a {gratEquivalent.toLocaleString("pt-BR")} bolsas
                  integrais
                </p>
                <div className="mt-5 pt-4 border-t border-[#e8e1d6] flex justify-between items-center text-[11px]">
                  ...
                  <strong className="text-[#34332f] bg-[#f4f2ea] px-2 py-0.5 rounded">
                    {gratTarget.toLocaleString("pt-BR")}%
                  </strong>
                </div>
              </Card>
              ...
              <Card
                className="p-6 transition-all hover:shadow-md cursor-pointer"
                onClick={() => router.push("/bolsistas")}
              >
                <div className="flex justify-between items-start mb-4">
                  <Eyebrow>Bolsas integrais</Eyebrow>
                  <span className="bg-[#f4f2ea] text-[#647078] px-2 py-0.5 rounded text-[10px] font-bold border border-[#e8e1d6]">
                    {bolsasRatio}
                  </span>
                </div>
                <h3 className="font-serif text-4xl text-[#34332f]">
                ...
                <p className="text-xs text-[#879087] mt-2">
                  de {bolsasTarget} bolsas mínimas projetadas
                </p>
                <div className="mt-5 pt-4 border-t border-[#e8e1d6] flex justify-between items-center text-[11px]">
                  <span className="text-[#879087]">
                    {bolsasMargin >= 0
                      ? "Margem de segurança"
                      : "Défice de bolsas"}
                  </span>
              ...
              <Card className="p-6 transition-all hover:shadow-md">
                <div className="flex justify-between items-start mb-4">
                  ...
                  <span className="border border-[#e8e1d6] text-[#647078] px-2 py-0.5 rounded text-[10px] font-bold">
                    {new Date().getFullYear()}
                  </span>
                </div>
                <h3 className="font-serif text-4xl text-[#34332f]">
                  {approvedDocs}{" "}
                  <span className="text-lg text-[#a38e7a]">/ {totalDocs}</span>
                </h3>
                <p className="text-xs text-[#879087] mt-2">
                  documentos validados pelo Compliance
                </p>
                <div className="mt-4 pt-3 flex items-center justify-between">
                  <div className="flex gap-4 text-[11px] font-bold">
                    <span className="flex items-center gap-1 text-[#4b8c78]">
                      <CheckCircle2 size={12} /> {approvedDocs}
                    </span>
                    <span className="flex items-center gap-1 text-[#c49a3c]">
                      <AlertTriangle size={12} /> {pendingDocs}
                    </span>
                  </div>
                </div>
                <button
                  onClick={() => router.push("/documentos?pending=true")}
                  className="mt-4 w-full text-left text-xs font-bold text-[#aa7a1d] hover:text-[#8a6317] hover:underline flex items-center gap-1"
                >
                  Ver o que aguarda conferência <ArrowRight size={12} />
                </button>
              </Card>
            </div>
            ...
            <div className="grid md:grid-cols-[1fr_1.5fr] gap-6">
              <Card className="p-6">
                <div className="flex justify-between items-end mb-6">
                  <div>
                    ...
                    <h3 className="font-serif text-lg text-[#34332f] mt-1">
                ...
                <div className="space-y-4">
                  {localPrazos.map((prazo: any) => {
                    ...
                    return (
                      <div
                        key={prazo.id}
                        className="flex gap-4 items-center border-b border-[#f4f2ea] pb-4 last:border-0 last:pb-0 group"
                      >
                        ...
                        <div className="flex-1">
                          ...
                          <p className="text-[10px] text-[#879087] mt-1 line-clamp-1">
                            {prazo.category || "Requisito Legal"}
                          </p>
                        </div>
                        <div className="flex flex-col gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                          <button
                            onClick={() =>
                              handleDismissPrazo(prazo.id, "concluido")
                            }
                            title="Marcar como concluído"
                            className="p-1.5 rounded-md bg-[#e7f3ee] text-[#4b8c78] hover:bg-[#bce0d3] transition-colors"
                          >
                            <Check size={12} />
                          </button>
                          <button
                            onClick={() =>
                              handleDismissPrazo(prazo.id, "excluido")
                            }
                            title="Remover alerta"
                            className="p-1.5 rounded-md bg-[#fdf0f0] text-[#d94444] hover:bg-[#f5c2c2] transition-colors"
                          >
                            <X size={12} />
                          </button>
                        </div>
              ...
              <Card className="p-6 flex flex-col justify-between">
                ...
                <div className="mt-2 flex items-end gap-2 text-[#34332f]">
                  ...
                  <span className="text-xs text-[#879087] mb-1">
                    projeção de gratuidade
                  </span>
                </div>
