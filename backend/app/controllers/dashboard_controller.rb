class DashboardController < ApplicationController
  ...
  def show
    ...
    status_counts = DocumentItem.where(institution_id: @institution.id).group(:status).count
    total_docs = status_counts.values.sum
    approved_docs = status_counts['aprovado'] || 0
    ...
    upcoming = Alerta.order(due_date: :asc).limit(4).map do |alerta|
      { id: alerta.id, name: alerta.titulo, category: alerta.category, due_date: alerta.due_date.to_s, tipo: alerta.tipo }
    end

    bolsas_counts = Bolsista.group(:scholarship_type).count
    integrais = bolsas_counts.select { |k, _| k.to_s =~ /Integral/i }.values.sum || 0
    parciais = bolsas_counts.select { |k, _| k.to_s =~ /Parcial/i }.values.sum || 0
    ...
    payload = {
      institution: { id: @institution.id, name: @institution.name, cnpj: @institution.cnpj },
      ...
      gratuidade: { current_percentage: grat_current, target_percentage: grat_target, equivalent_scholarships: equivalente_ofertado, growth: 3.2 },
      bolsas: { ratio: '1/5', current: integrais, target: bolsas_target },
